<script>
  ...
  function createNumoraInput() {
    ...
    numoraInputInstance = new NumoraInput(basicContainer, {
      decimalMaxLength: parseInt(decimalMaxLengthValue, 10),
  ...
  <div class="mb-8 p-4 border border-[#eaeaea] rounded bg-[#f8f9fa]">
    ...
    <div class="flex flex-col gap-4">
      <div class="flex flex-col gap-2">
        <label for="decimalMaxLength" class="text-sm text-[#666] font-medium">Max Decimals:</label>
        <input
          type="number"
          id="decimalMaxLength"
          min="0"
          max="20"
          bind:value={decimalMaxLengthValue}
          on:change={createNumoraInput}
          class="!p-2 !text-sm"
        />
      </div>
