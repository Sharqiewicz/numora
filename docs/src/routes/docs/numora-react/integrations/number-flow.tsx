...
const META_DESCRIPTION =
  'Integrate NumberFlow by Maxwell Barvian with Numora React to animate each digit while the real input keeps owning the keyboard, undo, IME, and string precision.';
    ...
    mentions: [
      {
        ...
        description:
          'Animated number transition library by Maxwell Barvian, with a React component (@number-flow/react) and a vanilla web component (number-flow).',
        url: LIB_URL,
  ...
  head: () => ({
    meta: [
      ...
      { name: 'description', content: META_DESCRIPTION },
      {
        name: 'keywords',
        content:
          'NumberFlow, NumberFlow React, @number-flow/react, animated numeric input React, Numora React, digit animation, number animation, Maxwell Barvian',
      },
  ...
  return (
    <div className="prose prose-invert max-w-xl">
      ...
      <p className="text-muted-foreground text-base leading-6">
        ...
        <code>&lt;input&gt;</code> still handles every keystroke; NumberFlow just renders what the
        user sees.
      </p>
      ...
      <ul>
        <li>
          <strong>Visible layer:</strong> a <code>&lt;NumberFlow&gt;</code> that animates the
      ...
      <p className="text-muted-foreground text-base leading-6">
        ...
        itself never animates - but because its text is transparent, you only see the NumberFlow
        layer.
      </p>
    ...
    <label className="relative inline-flex items-center text-4xl font-mono leading-none text-white">
      ...
      <ul>
        ...
        <li>
          ...
          locale uses <code>","</code>, which lines up with <code>thousandSeparator=","</code>.
        </li>
        <li>
          <strong>
            <code>mounted</code> gate
          </strong>{' '}
          - <code>@number-flow/react</code> is a client component (uses DOM APIs and{' '}
          <code>ResizeObserver</code>). Render a plain string at SSR; mount NumberFlow after
          hydration. In Next.js App Router, mark the page with <code>'use client'</code> instead.
        </li>
        <li>
          <strong>
            <code>text-transparent</code> + <code>caret-white</code>
          </strong>{' '}
          - hides the input's text but keeps the browser-rendered caret. The caret is the only thing
          the user sees from the real input.
        </li>
        <li>
          <strong>
            Matching typography on both layers (<code>text-4xl font-mono leading-none</code>)
          </strong>{' '}
          - the caret position is computed from the input's text layout. If fonts differ, the caret
          drifts.
        </li>
        <li>
          <strong>
            <code>tabular-nums</code> on the overlay
          </strong>{' '}
        ...
        <li>
          <strong>
            <code>m-0 p-0 border-0</code>
          </strong>{' '}
          - browser default input padding shifts the text origin. Zero padding aligns the input's
          text rendering with the overlay's text rendering.
        </li>
        <li>
          <strong>
            <code>selection:bg-white/25</code>
          </strong>{' '}
          - the input's text is transparent, so a default opaque selection rect would obscure the
          NumberFlow layer. A translucent selection lets the animated text show through.
        </li>
        <li>
          <strong>
            <code>placeholder-transparent</code>
          </strong>{' '}
          - the input's own placeholder would otherwise be visible. The overlay shows{' '}
          <code>"0"</code> as a fallback instead.
        </li>
        <li>
          <strong>
      ...
      <div className="bg-muted/50 border border-border rounded-lg p-4 my-4">
        <p className="text-muted-foreground text-base leading-6 m-0">
          ...
          beyond the visible <code>"1"</code>. This is unfixable without forking how NumberFlow
          renders. Brief and self-correcting once a fractional digit is typed.
        </p>
      </div>
      ...
      <div className="bg-muted/50 border border-border rounded-lg p-4 my-4">
        <p className="text-muted-foreground text-base leading-6 m-0">
          ...
          editable string (passed to your math / contracts) is untouched. The overlay is a display
          lie, not a state lie.
        </p>
      </div>
    ...
    <div>
      ...
      <ul>
        <li>
          ...
          pass it to anything that needs precision (math libraries, API calls, blockchain
          transactions). The overlay's <code>Number()</code> conversion is for display only.
        </li>
        <li>
          <strong>The input still owns the keyboard.</strong> Undo, redo, IME, paste, mobile{' '}
          <code>inputmode="decimal"</code>, native form submission all keep working in Pattern 1.
          The overlay is a display layer.
        </li>
        <li>
          <strong>
            Mirror typed decimals into <code>minimumFractionDigits</code>.
          </strong>{' '}
          Without this NumberFlow drops trailing zeros and the caret drifts off the end of the
          rendered number.
        </li>
        <li>
          <strong>Pattern 2 is the safe default.</strong> Animate readouts, not the editable field.
          No precision compromise, no caret drift. Pick Pattern 1 only when "the input itself
          animates" is the visual identity you want.
        </li>
        <li>
          <strong>RSC compatibility:</strong> <code>@number-flow/react</code> is a client component.
          Mark any Next.js App Router page using it with <code>'use client'</code>.
        </li>
      </ul>
      ...
      <p className="text-muted-foreground text-base leading-6">
        ...
        <code>Intl.NumberFormat</code> options for currency and grouping, and ships both a React
        component (<code>@number-flow/react</code>) and a vanilla web component (
      ...
      <p className="text-muted-foreground text-base leading-6">
        ...
        typed decimals through <code>minimumFractionDigits</code> so trailing zeros stay aligned
        with the caret.
      </p>
      ...
      <p className="text-muted-foreground text-base leading-6">
        ...
        <code>number-flow</code> (vanilla web component). Install with{' '}
        <code>npm install numora-react @number-flow/react</code>.
      </p>
    </div>
  );
}
