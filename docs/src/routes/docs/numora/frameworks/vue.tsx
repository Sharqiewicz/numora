...
const META_DESCRIPTION =
  'Vue numeric input library with thousand separators, decimal limits, paste sanitisation and cursor preservation. Drop Numora into Vue 3 or Nuxt with a tiny v-numora directive – framework-agnostic core, zero dependencies, 6.4 kb gzipped.';
  ...
  head: () => ({
    meta: [
      ...
      { name: 'description', content: META_DESCRIPTION },
      {
        name: 'keywords',
        content:
          'vue numeric input, vue 3 number input, vue input formatting, vue number format, vue input mask, nuxt numeric input, vue thousand separator, vue currency input, vue decimal input, v-numora directive, numora vue',
      },
  ...
  return (
    <div className="prose prose-invert max-w-xl">
      ...
      <p className="text-stone-400 text-base leading-6">
        ...
        six-line <code>v-numora</code> custom directive that mounts directly on an{' '}
        <code>&lt;input&gt;</code>. The core is framework-agnostic vanilla TypeScript, 6.4 kb
        gzipped, with zero runtime dependencies – the same engine works inside Vue 3, Nuxt 3,
...
<p className="text-stone-400 text-base leading-6">Raw value: {{ value }}</p>`}
      ...
      <p className="text-stone-400 text-base leading-6">
        ...
        <code>autocomplete</code>, <code>pattern</code>), and emits the raw, separator-free string
        from <code>onChange</code> – safe to feed straight into a ref, Pinia store, or VeeValidate
        field.
      </p>
...
.numora-overlay-host :deep(input::placeholder) { color: transparent; }`}
      ...
      <p className="text-stone-400 text-base leading-6">
        ...
        <code>v-model</code> means re-implementing cursor-positioning logic by hand - that&apos;s
