...
const META_DESCRIPTION =
  'Angular numeric input library with thousand separators, decimal limits, paste sanitisation and cursor preservation. Drop Numora into Angular with a standalone directive that composes with Reactive Forms ControlValueAccessor – framework-agnostic core, zero dependencies, 6.4 kb gzipped.';
  ...
  head: () => ({
    meta: [
      ...
      { name: 'description', content: META_DESCRIPTION },
      {
        name: 'keywords',
        content:
          'angular numeric input, angular number input, angular input formatting, angular number format, angular input mask, angular currency input, angular thousand separator, angular decimal input, angular reactive forms numeric, angular control value accessor, numora angular',
      },
  ...
  return (
    <div className="prose prose-invert max-w-xl">
      ...
      <p className="text-muted-foreground text-base leading-6">
        ...
        standalone directive that maps cleanly onto <code>ngOnInit</code> / <code>ngOnDestroy</code>
        . Because Numora&apos;s value is a string end-to-end, the directive composes with{' '}
        <code>ControlValueAccessor</code> and <code>FormControl&lt;string&gt;</code> without any
        number-coercion gymnastics – ideal for currency and DeFi flows that cannot tolerate IEEE 754
...
export class AmountFormComponent {
  ...
  protected readonly amount = new FormControl('0', { nonNullable: true });
      ...
      <p className="text-muted-foreground text-base leading-6">
        ...
        bridge means <code>writeValue</code> calls Numora&apos;s <code>setValue</code>,{' '}
        <code>registerOnChange</code> wires the raw string back into the form control, and{' '}
        <code>setDisabledState</code> toggles Numora&apos;s <code>disable()</code> /{' '}
  ...
  ngOnDestroy(): void {
    ...
    this.input?.removeEventListener('input', this.syncMorph);
      ...
      <p className="text-muted-foreground text-base leading-6">
        ...
        raw value is a string, which matches what <code>FormControl</code> stores – no
