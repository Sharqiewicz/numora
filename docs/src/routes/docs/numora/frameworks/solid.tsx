...
const META_DESCRIPTION =
  'SolidJS numeric input library with thousand separators, decimal limits, paste sanitisation and cursor preservation. Drop Numora into Solid or SolidStart with a tiny use:numora directive – framework-agnostic core, zero dependencies, 6.4 kb gzipped.';
  ...
  head: () => ({
    meta: [
      ...
      { name: 'description', content: META_DESCRIPTION },
      {
        name: 'keywords',
        content:
          'solid numeric input, solidjs numeric input, solid number input, solid input formatting, solid number format, solid input mask, solidstart numeric input, solid thousand separator, solid currency input, solid decimal input, numora solid',
      },
  ...
  return (
    <div className="prose prose-invert max-w-xl">
      ...
      <p className="text-stone-400 text-base leading-6">
        ...
        <code>use:numora</code> directive that mounts directly on an <code>&lt;input&gt;</code>.
        Write the raw value into a <code>createSignal</code> setter and Solid&apos;s fine-grained
        reactivity handles the rest. No <code>numora-solid</code> wrapper, no peer deps – the core
  ...
  return (
    <label>
      ...
      <p className="text-stone-400 text-base leading-6">
        ...
        <code>spellcheck</code>, <code>autocomplete</code>, <code>pattern</code>), and emits the
        raw, separator-free string from <code>onChange</code> – write it straight into a signal or
        store.
      </p>
...
.numora-overlay-host input::placeholder { color: transparent; }`}
      ...
      <p className="text-stone-400 text-base leading-6">
        ...
        sync across typing, paste, undo and redo. Torph respects <code>prefers-reduced-motion</code>{' '}
        automatically.
      </p>
      ...
      <p className="text-stone-400 text-base leading-6">
        ...
        means re-implementing the cursor-positioning logic by hand - that&apos;s the part Numora
        exists to solve.
      </p>
