...
export const Route = createFileRoute('/docs/numora/features/max-length')({
  head: () => ({
    meta: [
      ...
      { name: 'description', content: 'Cap numeric input length by raw digits with Numora. Thousand separators and decimal points are counted the way users actually think about length - not by formatted character count.' },
      { property: 'og:title', content: 'Max Length (Raw-Counted) | Numora' },
      { property: 'og:description', content: 'Cap numeric input length by raw digits with Numora. Separator-aware truncation for financial inputs.' },
      ...
      { name: 'twitter:description', content: 'Cap numeric input length by raw digits with Numora.' },
    ],
    links: [
      { rel: 'canonical', href: 'https://numeric-input.com/docs/numora/features/max-length' },
    ],
    scripts: [
      { type: 'application/ld+json', children: JSON.stringify([{ "@context": "https://schema.org", "@type": "TechArticle", "headline": "Max Length in Numora - Raw-Counted Numeric Input Length Limit", "description": "Cap numeric input length by raw digits with Numora. Thousand separators are excluded from the count, so the limit reflects the actual numeric value rather than the formatted display.", "url": "https://numeric-input.com/docs/numora/features/max-length", "author": { "@type": "Person", "name": "Kacper Szarkiewicz", "url": "https://x.com/sharqiewicz" } }, { "@context": "https://schema.org", "@type": "BreadcrumbList", "itemListElement": [{ "@type": "ListItem", "position": 1, "name": "Home", "item": "https://numeric-input.com" }, { "@type": "ListItem", "position": 2, "name": "Numora JS", "item": "https://numeric-input.com/docs/numora" }, { "@type": "ListItem", "position": 3, "name": "Max Length", "item": "https://numeric-input.com/docs/numora/features/max-length" }] }]) },
    ],
  }),
  component: MaxLength,
})
  ...
  return (
    <div className="prose prose-invert max-w-none">
      ...
      <p>
        ...
        characters; the user thinks of it as 7 digits. Numora's option counts the raw value
        the way users actually reason about length.
      </p>
...
// Next keystroke is blocked - raw length is already 7.`}
      ...
      <ul>
        ...
        <li>The decimal separator <em>is</em> counted (it's part of the raw value)</li>
        <li>A leading <code>-</code> sign <em>is</em> counted</li>
        <li>Truncation happens from the right; if it would leave a trailing decimal separator, that separator is also stripped (<code>"1234.56"</code> capped at 5 → <code>"1234"</code>)</li>
        <li>If truncation would leave a bare <code>"-"</code>, the result is <code>""</code></li>
      </ul>
