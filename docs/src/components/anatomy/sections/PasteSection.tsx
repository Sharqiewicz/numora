  ...
  return (
    <section id="paste" className="space-y-4 scroll-mt-24">
      ...
      <p>
        ...
        <code>inputType: 'insertFromPaste'</code>. Numora returns immediately - the dedicated{' '}
        <code>paste</code> handler takes over because it has access to the full{' '}
        <code>ClipboardData</code> object.
      </p>
      <p>The paste handler:</p>
      <ol>
        <li>
          <code>e.preventDefault()</code> - block the native paste outright.
        </li>
        <li>
          Read <code>e.clipboardData.getData('text/plain')</code>.
        </li>
        ...
        <li>
          ...
          <code>shouldRemoveThousandSeparators: true</code> - unconditionally, because clipboard
          text may carry separators from a foreign locale (German <code>"1.234,56"</code>) that
          would otherwise be parsed as decimals.
        </li>
        <li>
          Set <code>input.value</code> directly and reposition the cursor.
        </li>
        <li>
          Dispatch a synthetic <code>input</code> event so consumers see <code>onChange</code>.
        </li>
      </ol>
