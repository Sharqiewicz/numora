...
export function FormatModeSection() {
  return (
    <section id="format-mode" className="space-y-4 scroll-mt-24">
      ...
      <p>
        Numora applies thousand separators in one of two modes. <code>FormatOn.Change</code> groups
        every keystroke; the user sees commas appear as they type. <code>FormatOn.Blur</code> groups
        only when the input loses focus; while editing, the value reads as a plain unformatted
        string. Both modes use the same pipeline - only the <em>moment</em> of formatting differs.
      </p>
      <p>
        Type <code>1234567.89</code> in both inputs to feel the difference:
      </p>
      <InputPair
        left={{
